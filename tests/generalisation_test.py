...
generaliser = generalisation.GeneraliseData
...
def test_format_coordinates_valid_input():
    ...
    lat, lon = generaliser.format_coordinates(input_data)
...
def test_format_coordinates_invalid_input(invalid_input):
    ...
    with pytest.raises((ValueError, IndexError)):
        generaliser.format_coordinates(pd.Series([invalid_input]))
...
def test_generalise_spatial_invalid_resolution(invalid_resolution):
    ...
    with pytest.raises(ValueError):
        generaliser.generalise_spatial(lat, lon, invalid_resolution)
...
def test_generalise_spatial_mismatched_series():
    ...
    with pytest.raises(Warning):
        generaliser.generalise_spatial(lat, lon, resolution)
...
def test_random_coordinates():
    ...
    parsed_lat, parsed_lon = generaliser.format_coordinates(coord_strings)
    assert len(parsed_lat) == n_points
    assert len(parsed_lon) == n_points
    
    # Test generalise_spatial
    h3_indexes = generaliser.generalise_spatial(parsed_lat, parsed_lon, resolution)
